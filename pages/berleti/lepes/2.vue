...
<template>
    <section class="max-w-2xl mx-auto">
        <AppTooltip :width="300" :cross-axis="-10" label="Ezt jó ha tudod!">
            <template #content>
                <h5
                    class="pb-2 text-white border-b dark:text-neutral-900 border-b-neutral-400"
                >
                    Eltérések a válaszokban
                </h5>
                <p class="text-xs font-normal tracking-wide text-neutral-200">
                    Egyes kérdéskörök jogszabályi alapállapotától csak a Bérlő,
                    más kérdéskörökben pedig akár a Bérlő, akár a Bérbeadó
                    javára is eltérhetsz. Összeszedtük a kérdésköröket, és előre
                    megírt eltérési lehetőségeket ajánlunk fel neked a
                    jogszabályi lehetőségeknek megfelelően.
                </p>
                <p class="text-xs font-normal tracking-wide text-neutral-200">
                    “§” jellel jelöltük, hogy melyik a jogszabályi alapállapot.
                    Ha szeretnél valamelyik fél javára eltérni, válaszd a
                    számodra kedvezőbb megfogalmazást. Ha kiválasztottál egy
                    eltérést, az ahhoz tartozó szerződéses rendelkezés akkor is
                    szerepelni fog a szerződésedben, ha fentebb nem jelölted be
                    egyik háttérjogszabályt sem.
                </p>
                <p class="text-xs font-normal tracking-wide text-neutral-200">
                    Ha egyik kérdésben sem szeretnél eltérni, csak gyorsan
                    szeretnél egy bérleti szerződést, alul kattints a Tovább
                    gombra hogy minden kérdéskörben a jogszabályi alapállapot
                    kerüljön kiválasztásra. Így egyszerűen a végére érhetsz és
                    nem kell minden kérdésen egyesével átmenned.
                </p>
            </template>
        </AppTooltip>
        ...
        >
            <InputRadioBase
                ...
                @update:model-value="
                    (optionId: string) => updateQuestions(optionId, 3)
                "
            />
            <InputRadioBase
                v-model="Q5Value"
        ...
        >
            <template #hint>
                ...
                >
                    <template #content>
                        ...
                        >
                            A Polgári Törvénykönyv szerint bármilyen
                            pénztartozás fizetési határidejének elmulasztása
                            esetén késedelmi kamatot kell fizetni.
                        </p>
                    </template>
                </AppTooltip>
            </template>
            <InputRadioBase
                ...
                @update:model-value="
                    (optionId: string) => updateQuestions(optionId, 4)
                "
            />
            <InputRadioBase
                v-model="Q3Value"
            ...
            <InputRadioText
                ...
                @update:text-value="
                    (textValue: string) =>
                        updateQuestions(Q3Value, 4, textValue)
                "
            />
        </QuestionBlock>
        ...
        >
            ...
            <InputRadioText
                ...
                @update:text-value="
                    (textValue: string) =>
                        updateQuestions(Q11Value, 9, textValue)
                "
            />
        </QuestionBlock>
    </section>
