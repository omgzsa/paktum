...
const { getQuestion, updateQuestions, updateQuestionsWithTwoParams } =
    contractStore
    ...
    <section class="max-w-2xl mx-auto">
        ...
        >
            <template #hint>
                ...
                >
                    <template #content>
                        ...
                        >
                            Ha a szerződés rendkívüli felmondással szűnik meg,
                            annak oka jellemzően az, hogy az egyik fél nem
                            teljesítette valamilyen kötelezettségét, vagy a
                            szerződéssel ellentétes magatartást tanúsított (pl.
                            nem fizetett bérleti díjat, nem javította ki a
                            Bérlemény hibáját, vagy az együttélés
                            követelményeivel kirívóan ellentétes magatartást
                            tanúsított), és ezért a másik fél rendkívüli
                            felmondással felmondott.
                        </p>
                    </template>
                </AppTooltip>
            </template>
            <InputRadioTextText
                ...
                @update:model-value="
                    (optionId: string) =>
                        updateQuestions(Q27Value, 25, optionId)
                "
                @update:text-values="
        ...
        >
            <InputRadioText
                ...
                :width="45"
                @update:model-value="
                    (optionId: string) => updateQuestions(optionId, 26)
                "
                @update:text-value="
                    (textValue: string) =>
                        updateQuestions(Q28Value, 26, textValue)
                "
            />
            <InputRadioBase
                v-model="Q28Value"
